...
class SOMBenchmark(BenchmarkABC):
    ...
    def winning_neuron(self, data, t):
        ...
        for row in range(self.num_rows):
            for col in range(self.num_cols):
                distance = self.e_distance(self.som[row][col], data[t])
    ...
    def predict(self, label_map, test_x):
        # test data
        # using the trained som, search the winning node of corresponding to the test data
        # get the label of the winning node
        winner_labels = []
        for t in range(test_x.shape[0]):
            winner = self.winning_neuron(test_x, t)
            row = winner[0]
            col = winner[1]
    ...
    def collect_labels(self, train_x, train_y):
        # collecting labels
        label_data = train_y
        ...
        for t in range(train_x.shape[0]):
            winner = self.winning_neuron(train_x, t)
                ...
                for col in range(self.num_cols):
                    if self.m_distance([row, col], winner) <= neighbourhood_range:
                        self.som[row][col] += learning_rate * (
                                train_x[t] - self.som[row][col])  # update neighbour's weight
    ...
    def evaluate(self, dataset: pd.DataFrame) -> float:
        ...
        np.random.seed(40)
        self.som = np.random.random_sample(size=(self.num_rows, self.num_cols, num_dims))
        self.train(train_x)
        map = self.collect_labels(train_x, train_y)
        label_map = self.construct_map(map)
        winner_labels = self.predict(label_map, test_x)
